...
The goal of [cnd](https://jmbarbone.github.io/cnd/) is to provide easy,
customized classes for your `conditions`.
...
[cnd](https://jmbarbone.github.io/cnd/) from
...
and messaging, primarily within the context of
...
be sent to the
[`stderr()`](https://rdrr.io/r/base/showConnections.html). You’ll
notice, too, that [`warning()`](https://rdrr.io/r/base/warning.html) and
[`message()`](https://rdrr.io/r/base/message.html) call
[`withRestarts()`](https://rdrr.io/r/base/conditions.html), which
contain [`signalCondition()`](https://rdrr.io/r/base/conditions.html)
...
[cnd](https://jmbarbone.github.io/cnd/) inserts itself into these
processes by allowing users to define new condition objects to be
signaled and with greater control over messaging.

## Example

The workhorse of [cnd](https://jmbarbone.github.io/cnd/) is
[`condition()`](https://jmbarbone.github.io/cnd/reference/condition.md),
which is a special function of class
[`cnd::condition_progenitor`](https://jmbarbone.github.io/cnd/reference/condition.md),
which returns other special functions of class
[`cnd::condition_generator`](https://jmbarbone.github.io/cnd/reference/condition.md).
The
[`cnd::condition_generator`](https://jmbarbone.github.io/cnd/reference/condition.md)
...
Use
[`condition()`](https://jmbarbone.github.io/cnd/reference/condition.md)
to create a *generator*, then use that *generator* within your
functions:
...
The resulting
[`cnd::condition_generator`](https://jmbarbone.github.io/cnd/reference/condition.md)
object can also take parameters that are used in creating a custom
message.
...
conditions. This environment must exist within your package, an
[cnd](https://jmbarbone.github.io/cnd/) will be able to find this and
use it to connect your conditions to your functions and to other
outputs.
...
> for an example of how to create a new registry and assign conditions
> to the registry.

### Assigning conditions

[`condition()`](https://jmbarbone.github.io/cnd/reference/condition.md)
has an argument for `exports`, which you can set to any function which
you want to relate with any specific conditions.
...
In your package, add
[`cnd_exports()`](https://jmbarbone.github.io/cnd/reference/cnd_exports.md)
to an `R/` script. This should be executed after all your conditions and
their functions are created. This will add a new `"conditions"`
attribute to your functions as well as a new
`"cnd::conditioned_function"` class. The new class specifically updates
the [`print()`](https://rdrr.io/r/base/print.html) method to show the
...
will create a new `{package}-cnd-conditions.R` file for all conditions
you have assigned to your package. Simply run the command when
developing to generate a file listing all conditions. You can also
include this after your call to
[`cnd_exports()`](https://jmbarbone.github.io/cnd/reference/cnd_exports.md)
to ensure that all conditions are documented. The file is written for
[roxygen2](https://roxygen2.r-lib.org/) to generate the `Rd` files for
your package.
...
[`cnd_section()`](https://jmbarbone.github.io/cnd/reference/cnd_document.md)
function to grab all the conditions from a single functions and print
...
You can retrieve any `conditions` that are created with
...
[`cnd()`](https://jmbarbone.github.io/cnd/reference/condition.md) is a
special function which will use the appropriate signaling and handling
function based on type of `condition` provided. When the condition’s
type is `"error"` or `"warning"`,
[`cnd()`](https://jmbarbone.github.io/cnd/reference/condition.md) passes
these directly through [`stop()`](https://rdrr.io/r/base/stop.html) and
[`warning()`](https://rdrr.io/r/base/warning.html), respectively. Both
of these functions have
[`.Internal()`](https://rdrr.io/r/base/Internal.html) calls (i.e.,
`.dfltStop()` and `.dfltWarn()`), which makes would make them difficult
to replicate. However,
[`message()`](https://rdrr.io/r/base/message.html) does not, and thus an
...
[`message()`](https://rdrr.io/r/base/message.html) uses a handler which
simply collapses the message vector into a single string. Because of
...
By contrast, the handlers invoked in
[`cnd()`](https://jmbarbone.github.io/cnd/reference/condition.md) will
...
To get the a simpler message, you can use the
[`options()`](https://rdrr.io/r/base/options.html) function to change
...
[`stdout()`](https://rdrr.io/r/base/showConnections.html). Using
[`cat()`](https://rdrr.io/r/base/cat.html) can sometimes create noise
that you’d rather suppress. Because
[`cnd()`](https://jmbarbone.github.io/cnd/reference/condition.md) uses
an internal handler for `message` and `condition` types, a condition is
signaled with `singalCondition()`, which can then be caught with calling
